...
{
    private static readonly string ClientCharacteristicConfigurationUUID = "00002902-0000-1000-8000-00805f9b34fb";
    ...
    public void OnCharacteristicRead(BluetoothGattCharacteristic characteristic, GattStatus status)
    {
        lock (_lock)
        {
            _readCompletionSource?.TrySetResult(characteristic.GetValue(0));
        }
    }

    public void OnCharacteristicWrite(BluetoothGattCharacteristic characteristic, GattStatus status)
    {
        lock (_lock)
        {
    ...
    public void OnDescriptorWrite(BluetoothGattDescriptor descriptor, GattStatus status)
    {
        lock (_lock)
        {
    ...
    public void OnCharacteristicChanged(BluetoothGattCharacteristic characteristic)
    {
        lock (_lock)
        {
            var guid = new Guid(characteristic.Uuid);
            var data = characteristic.GetValue(0);

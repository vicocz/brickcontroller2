...
namespace BrickController2.DeviceManagement
{
    internal class SBrickDevice : BluetoothDevice
    {
        ...
        {
            var valid = services.Any(s => s.Uuid == SERVICE_UUID_REMOTE_CONTROL &&
                s.ContainsCharacteristic(CHARACTERISTIC_UUID_REMOTE_CONTROL) &&
                s.ContainsCharacteristic(CHARACTERISTIC_UUID_QUICK_DRIVE));
        ...
        private async Task<bool> SendOutputValuesAsync(int[] v, int[] dir, CancellationToken token)
        {
            try
            {
                ...
                return await _bleDevice!.WriteAsync(CHARACTERISTIC_UUID_QUICK_DRIVE, sendOutputBuffer, token);
            }
            catch
            {
                return false;
        ...
        private async Task ReadDeviceInfo(CancellationToken token)
        {
            ...
            await _bleDevice.WriteAsync(CHARACTERISTIC_UUID_REMOTE_CONTROL, new byte[] { 0x0f, 0x08 }, token);
            var voltageBuffer = await _bleDevice!.ReadAsync(CHARACTERISTIC_UUID_REMOTE_CONTROL, token);

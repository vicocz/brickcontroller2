...
public static class CryptTools
{
    ...
    public static byte[] DecryptRfPayload(byte[] seed, int headerLength, int dataLength, int headerOffset, byte ctxValue1, byte ctxValue2, byte[] rfPayload)
    {
        ...
        {
            header[i] = Invert8(resultBuffer[headerOffset + i]);
        }

        for (int i = 0; i < seedLength; i++)
        {
            seedReversed[i] = Invert8(resultBuffer[headerOffset + headerLength + i]);
        }
        ...
        ushort actualCrc = CheckCRC16(seedOriginal, data);

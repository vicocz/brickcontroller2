...
namespace BrickController2.UI.Controls
{
    ...
    public partial class DeviceChannelSelector : ContentView
    {
        ...
        {
            if (bindable is DeviceChannelSelector dcs && value is Device device && ReferenceEquals(dcs.Device, device))
            {
                // enforce update even when the same Device reference is re-set (e.g. TechnicMove PlayVM mode change),
                // since in that case the propertyChanged callback won't fire.
